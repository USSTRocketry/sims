...
def main_parachute_opening(apogee_detected:bool, altitude:float) -> bool:
    # Main now deploys immediately at apogee (same condition as Drogue),
    # rather than waiting until descending below a given altitude.
    return apogee_detected


# Basic analysis info
filename = BASE_DIR / "Up2Down_Ballistic"
print("Filename is:")
print(filename)
...
for setting in flight_settings(analysis_parameters, number_of_simulations):
    ...
    # Ballistic case: BOTH parachutes DISABLED. No deployments occur at
    # all - the rocket free-falls from apogee straight to ground impact.
    # add_parachute calls are commented out rather than deleted so they
    ...
    try:
        ...
        # Guard against flights that hit max_time before the impact event
        # was actually detected. In that case RocketPy never overwrites
        # x_impact/y_impact, leaving them at their default of (0, 0) -
        # i.e. the launch point - which would silently corrupt the
        # landing dispersion plot. Treat these as failed runs instead.
        if rocket_flight.t_final >= rocket_flight.max_time - 1e-6:
            raise RuntimeError(
                f"Flight reached max_time ({rocket_flight.max_time}s) "
                "before a real ground impact was detected; "
                "impact_x/impact_y would be invalid (0, 0)."
            )
...
output_folder_svg = str(BASE_DIR / "images_ballistic" / "svg")
os.makedirs(output_folder_svg, exist_ok=True)

# Create an output folder for pickle files
output_folder_pickle = str(BASE_DIR / "images_ballistic" / "pickle")
...
# DROGUE PARACHUTE TRIGGER TIME / INFLATED TIME / INFLATED VELOCITY
#
# Ballistic case: there are no parachute deployments at all, so these
# three plots are skipped entirely. dispersion_results["drogue_*"]
# columns are all zeros (see export_flight_data's fallback for flights
# with zero parachute_events), and fitting/plotting a normal curve to
# an all-zero column produces a divide-by-zero / nan in the fitted PDF
# overlay. Printing a quick note instead of generating a broken plot.
print("Ballistic case: no parachute events occurred - skipping "
      "drogue trigger time / inflated time / inflated velocity plots.")
